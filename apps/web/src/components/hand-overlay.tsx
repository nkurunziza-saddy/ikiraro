...
export default function HandOverlay({ tracking }: { tracking: CameraTrackingState }) {
  ...
  useEffect(() => {
    ...
    for (const index of [4, 8, 12, 16, 20]) {
      ...
      context.beginPath();
      context.arc(point.x * width, point.y * height, 4.5, 0, Math.PI * 2);
      context.fill();
    }
    ...
    if (isDetected && tracking.landmarks[9]) {
      ...
      context.fillStyle = "rgba(6, 12, 18, 0.88)";
      context.beginPath();
      context.roundRect(x - 22, y - 20, 44, 44, 10);
      context.fill();
      ...
      context.scale(-1, 1);
      context.fillText(detectedSign, 0, 0);
      context.font = "700 10px DM Sans Variable";
      context.fillText(`${Math.round(confidence * 100)}%`, 0, 26);
      context.restore();
    }
