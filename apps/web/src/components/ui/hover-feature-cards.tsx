  ...
  const inner = (
    <motion.div
      ...
      className={cn(
        ...
        item.soon ? "opacity-80 cursor-not-allowed" : item.href ? "cursor-pointer" : "",
      )}
    >
      <div
        className={cn(
      ...
      >
        {item.soon && (
          <span className="absolute top-3 end-3 z-10 text-xs text-muted-foreground border rounded-full px-2 py-1 bg-card">
            Coming soon
          </span>
        )}
        <div
      ...
      >
        <div className="py-3 px-5 relative border-t-0 rounded-b-3xl border">
          <div className="pointer-events-none w-[103%] bg-gradient-to-b from-background to-transparent h-12 absolute -top-1 -start-1" />
          <p className="text-sm font-base text-muted-foreground">{item.description}</p>
        </div>
      </motion.div>
    </motion.div>
  );
...
function HoverFeatureCards({ items, className, renderLink }: HoverFeatureCardsProps) {
  return (
