  ...
  return (
    <aside className="w-full max-w-[320px] border-r border-border flex flex-col shrink-0 relative z-10 bg-background/50 backdrop-blur-md h-full">
      ...
      <div className="px-5 py-6 shrink-0">
        <Link
          to="/"
          className="text-[15px] font-medium transition-transform active:scale-[0.985] duration-200 ease-out inline-block"
        >
          ikiraro
        </Link>
      </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <TabsContent
                value="stream"
                forceMount
                className={cn("space-y-3 mt-0", activeTab !== "stream" && "hidden")}
              >
                <div>
              ...
              <TabsContent
                value="metadata"
                forceMount
                className={cn("space-y-3 mt-0", activeTab !== "metadata" && "hidden")}
              >
                <WorkflowSteps
                  steps={[
                    {
              ...
              <TabsContent
                value="history"
                forceMount
                className={cn("space-y-3 mt-0", activeTab !== "history" && "hidden")}
              >
                  ...
                  <div className="space-y-2">
                    <ModeSelector
                      ...
                      items={ACCESSIBILITY_OPTIONS.map((option) => ({
                        ...option,
                        description:
                          option.value === accessMode ? "Active mode" : option.description,
                      }))}
                      onSelect={(value) => setAccessMode(value as any)}
                      className={cn(
                        "cursor-pointer transition-opacity",
                        accessMode && "opacity-100",
                      )}
                    />
                  </div>
                </div>
