 ...
 *
 * Usage:
 *   bun packages/engine/scripts/build-centroids.ts <landmarks-dir> <train.csv>
 *
...
`.trim();
  ...
  for (const row of rows) {
    ...
    if (t.includes("right") || !fm.has(row.idx)) {
      if (row.idx >= 0 && row.idx < 21 && isFinite(row.x) && isFinite(row.y) && isFinite(row.z)) {
 ...
 * Generated by: bun packages/engine/scripts/build-centroids.ts
 */
