...
export function evaluateHandGeometry(landmarks: HandLandmarks): HandGeometryQuality {
  ...
  const normalizedPalmWidth = palmWidth > 0 ? palmWidth : 1;
  const middleFingerRatio = middleFingerLength / normalizedPalmWidth;
  const wristToMiddleTipRatio = wristToMiddleTip / normalizedPalmWidth;
  const palmTriangleRatio = palmTriangleArea / (normalizedPalmWidth * normalizedPalmWidth);
  const minSegmentRatio = Math.min(
    ...FINGER_SEGMENTS.map(
      ([start, end]) => getDistance(landmarks[start]!, landmarks[end]!) / normalizedPalmWidth,
    ),
  );
