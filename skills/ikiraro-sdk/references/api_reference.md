...
interface IkiraroReactClient {
  ...
  useIkiraroPlugin: <K extends keyof PluginRegistry>(pluginName: K) => PluginRegistry[K] | undefined;
...
```typescript
type MotionType =
  | "none" | "shake" | "arc" | "salute" | "forward-push" | "outward-sweep"
  | "pull-back" | "chest-pat" | "two-hand-tap" | "music-sweep" | "wrist-twist"
  | "tap" | "circle" | "z-trace" | "j-trace" | "g-push" | "h-slide"
  | "d-arc" | "n-dip" | "k-push" | "wave";
```
...
```typescript
type ArmTarget = {
  rArmX?: number; rArmZ?: number; rArmY?: number;
  rForeX?: number; rForeZ?: number; rForeY?: number;
  rHandX?: number;
  lArmX?: number; lArmZ?: number; lArmY?: number;
  lForeX?: number; lForeZ?: number; lForeY?: number;
  lHandX?: number;
};
```
